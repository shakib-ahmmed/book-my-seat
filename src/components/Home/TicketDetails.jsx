...
const TicketDetails = () => {
    ...
    return (
        <div className="min-h-screen p-6 md:p-10 lg:p-16 bg-base-100 dark:bg-base-200 transition-colors duration-300">
            <div className="max-w-6xl mx-auto bg-white dark:bg-[#3A0E0E] border border-gray-300 dark:border-[#BA0C10] rounded-2xl shadow-lg p-6 md:p-10 space-y-8 transition-colors duration-300">
                {/* TITLE */}
                <h1 className="text-4xl font-extrabold text-black dark:text-[#FDDB1A]">
                    {ticket.title}
                </h1>
                    ...
                    {ticket.images?.length ? (
                        ticket.images.map((img, idx) => (
                            <img
                                ...
                                className="w-full md:w-1/3 h-60 md:h-72 object-cover rounded-xl border border-gray-300 dark:border-[#BA0C10]"
                            />
                        ))
                    ) : (
                        <img
                            src={ticket.image}
                            alt={ticket.title}
                            className="w-full h-72 object-cover rounded-xl border border-gray-300 dark:border-[#BA0C10]"
                        />
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-gray-800 dark:text-gray-200">
                    {/* OVERVIEW */}
                    <section>
                        <h2 className="text-xl font-bold text-black dark:text-[#FDDB1A] mb-2">Overview</h2>
                    ...
                    {/* SPECS */}
                    <section>
                        <h2 className="text-xl font-bold text-black dark:text-[#FDDB1A] mb-2">Ticket Info</h2>
                    ...
                    {/* DEPARTURE */}
                    <section>
                        <h2 className="text-xl font-bold text-black dark:text-[#FDDB1A] mb-2">Departure</h2>
                    ...
                    {/* REVIEWS */}
                    <section>
                        <h2 className="text-xl font-bold text-black dark:text-[#FDDB1A] mb-2">Reviews</h2>
                        {ticket.reviews?.length ? (
                            ticket.reviews.map((r, idx) => (
                                <div key={idx} className="mb-2 border-b border-gray-300 dark:border-[#BA0C10] pb-2">
            ...
            ${soldOut || departed
                ...
                {relatedTickets.length > 0 && (
                    <section className="mt-8">
                        <h2 className="text-2xl font-bold text-black dark:text-[#FDDB1A] mb-4">
                            Related Tickets
                        </h2>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {relatedTickets.slice(0, 3).map((rt) => (
                                <Link key={rt._id} to={`/tickets/${rt._id}`}>
                                    <div className="bg-white dark:bg-[#5C0809]/70 border border-gray-300 dark:border-[#BA0C10] rounded-xl overflow-hidden hover:scale-105 transition-colors duration-300 cursor-pointer">
                                        <img
                                            src={rt.image}
                                            alt={rt.title}
                                            className="w-full h-40 object-cover"
                                        />
                                        <div className="p-2 text-black dark:text-[#E8D351] font-semibold">
                                            {rt.title}
                                        </div>
                                    </div>
                                </Link>
